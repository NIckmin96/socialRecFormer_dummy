...
class DatasetMaking:
    def __init__(self, dataset, seed, user_seq_len, item_seq_len, return_params, train_augs):
        # self.args = args
        ...
        self.random_walk_train = utils.generate_social_random_walk_sequence(data_path, self.social_train, self.user_degree_train, walk_length=user_seq_len, data_split_seed=seed, split='train', return_params=return_params, train_augs=train_augs)
        self.random_walk_valid = utils.generate_social_random_walk_sequence(data_path, self.social_valid, self.user_degree_valid, walk_length=user_seq_len, data_split_seed=seed, split='valid', return_params=return_params)
        self.random_walk_test = utils.generate_social_random_walk_sequence(data_path, self.social_test, self.user_degree_test, walk_length=user_seq_len, data_split_seed=seed, split='test', return_params=return_params)
        ############# 모델 입력을 위한 최종 데이터셋 구성
        self.total_train = utils.generate_input_sequence_data(data_path=data_path, user_df=self.random_walk_train, item_df=self.user_item_table_train, seed=seed, split='train', random_walk_len=user_seq_len, item_seq_len=item_seq_len, return_params=return_params, train_augs=train_augs)
        self.total_valid = utils.generate_input_sequence_data(data_path=data_path, user_df=self.random_walk_valid, item_df=self.user_item_table_valid, seed=seed, split='valid', random_walk_len=user_seq_len, item_seq_len=item_seq_len, return_params=return_params)
        self.total_test = utils.generate_input_sequence_data(data_path=data_path, user_df=self.random_walk_test, item_df=self.user_item_table_test, seed=seed, split='test', random_walk_len=user_seq_len, item_seq_len=item_seq_len, return_params=return_params)